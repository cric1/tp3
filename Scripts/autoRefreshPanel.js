        ...
        if (!this.paused) {
            $.ajax({
                ...
                statusCode: {
                    401: function () {
                        if (EndSessionAction != "")
                            window.location = EndSessionAction + "?message=Votre compte a été bloqué par le webmestre ou a été fermée à cause d'une autre session avec votre compte. !&success=false";
                        else
