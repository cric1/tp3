...
namespace JsonDemo.Controllers
{
    ...
    public class StudentsController : Controller
    {
        ...
        public ActionResult GetStudent(bool forceRefresh = false)
        {
            if (forceRefresh || DB.Students.HasChanged)
            {
                if (Session["id"] != null)
                {
                    Student student = DB.Students.Get((int)Session["id"]);
        ...
        {
            if (forceRefresh || DB.Registrations.HasChanged || DB.Courses.HasChanged)
            {
                if (Session["id"] != null)
                {
                    Student student = DB.Students.Get((int)Session["id"]);
